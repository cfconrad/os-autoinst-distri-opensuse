...
our %registries = (
    ...
    opensuse => {
        ...
        '15.3' => {
            ...
            totest   => sub {
                if (check_var('ARCH', 'x86_64') || check_var('ARCH', 'aarch64') || check_var('ARCH', 'ppc64le') || check_var('ARCH', 's390x')) {
    ...
    microos => {
        ...
        '15.1' => {
            ...
            totest   => sub {
                if (check_var('ARCH', 'x86_64')) {
                    'registry.opensuse.org/opensuse/leap/15.1/images/totest/containers/opensuse/leap:15.1';
                } elsif (check_var('ARCH', 'aarch64') || check_var('ARCH', 'arm')) {
                    'registry.opensuse.org/opensuse/leap/15.1/arm/images/totest/containers/opensuse/leap:15.1';
                } else {
                    '';
                }
...
sub get_suse_container_urls {
    ...
    if (check_image_arch_in_registry($args{distri}, $args{version}, $args{arch})) {
        push @untested_images, $registries{$args{distri}}{$args{version}}{totest}->();
        push @released_images, $registries{$args{distri}}{$args{version}}{released}->();
    } else {
        die("Unknown combination of distro/arch.");
    }
