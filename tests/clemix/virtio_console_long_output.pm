...
sub run {
    ...
    for (my $i = 0; $i < $repeat; $i++) {
        my $output = undef;
        eval {
            script_output("echo 'RUN: $i'", quiet => 1, timeout => 10);
            $output = script_output('cat ' . $filename, quiet => 1, timeout => 10, proceed_on_failure => 1);
        };
        if (!defined($output)) {
            print("CLEMIX Start debugging\n");
            print("CLEMIX close console\n");
            $console->disable();
            print("CLEMIX console disabled\n");
            $console = $self->select_serial_terminal;
            print("CLEMIX console activated\n");
            my $ret = script_output("echo 'RECOVER CONSOLE'");
            unless ($ret) {
                die("Ooops: this should not happen! Timeout appear, check serial_terminal.txt for last output\n" . $@);
            } else {
                record_info("CONSOLE RECOVER", "", result => 'fail');
                next;
            }
        }
