...
LOCAL_IP=${HOST3_IP:?MISSING HOST2_IP}
...
setup_network "$CTRL_ETH" "$CTRL_VLAN" "$HOST3_IP"
...
ovs-vsctl add-port br-int vif3 -- set Interface vif3 type=internal external_ids:iface-id=port-host3
ip netns add ns3
ip link set vif3 netns ns3
ip netns exec ns3 ip link set vif3 address 02:00:00:00:00:13
ip netns exec ns3 ip addr add $HOST3_TEST_IP/24 dev vif3
ip netns exec ns3 ip link set vif3 up
ip netns exec ns3 ip route add default via $ROUTER3_TEST_IP

echo "ip netns exec ns3 iperf3 -s"
